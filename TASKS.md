...
- [x] **Checkpoint `m1-s2-repository-contracts-merged` — canonical handoff documentation prepared and verified** — [HANDOFF.md](HANDOFF.md) (the model-neutral checkpoint and resume document) created and the checkpoint protocol added to [CLAUDE.md](CLAUDE.md); documentation-only and locally verified via `scripts/verify.sh` — _prepared 2026-07-30; authorizes no slice. Per the checkpoint protocol, checkpoint closure additionally requires this documentation PR to merge to `main`; S3 release remains a separate explicit human decision after that_
- [!] S3 — Fixture suite v1 in `fixtures/demo-company/` — _gated on slice release; **next up, but unreleased**: release requires the S2 checkpoint recorded (done — PR #10 merged) and an explicit human release recorded here_
