...
**M0 complete (2026-07-22). M1 implementation explicitly authorized (2026-07-23) — active, slice-gated.** The documentation set is approved, the M0 tooling ADRs (0001–0013 in [docs/adr/](docs/adr/README.md)) are formally Accepted, and the full M0 foundation exists and passes CI: the monorepo workspace, the backend API shell (`apps/api`), the web application shell (`apps/web`), the shared package shells (`packages/*`), the browser acceptance suite (`tests/acceptance`), the verification tooling (`scripts/bootstrap.sh`, `scripts/verify.sh`), and the GitHub Actions workflow that runs it. The M1 architecture baseline is settled ([docs/m1-plan.md](docs/m1-plan.md) approved, ADRs 0014–0020 Accepted; ADR-0014/0015 amended by [ADR-0019](docs/adr/0019-subject-identity-and-assertion-claims.md), ADR-0017 amended by [ADR-0020](docs/adr/0020-m1-inventory-and-search-semantics.md)), and M1 implementation is authorized to execute that plan. **Slice S1 (domain schemas in `packages/shared`) is complete — merged through PR #7 on 2026-07-29. Slice S2 (repository interfaces + storage-agnostic contract-test suite skeleton) is complete — human-approved 2026-07-29 and merged through PR #10; the S2 checkpoint/HANDOFF protocol merged through PR #11 at `a7a997d` (2026-07-30). Slice S3 (fixture suite v1) was explicitly human-authorized 2026-07-30 — its implementation may begin only after the documentation PR recording that authorization merges to `main` and `main` is synchronized locally; once merged, S3 is the only active authorized implementation slice.** See [TASKS.md](TASKS.md) for exact per-task status and [HANDOFF.md](HANDOFF.md) for the current checkpoint.

Permitted work right now, and nothing beyond it:

- **Maintenance and corrections to the completed M0 foundation and the merged S1/S2 contract surface** (bug fixes, documentation fixes, dependency/security maintenance within the accepted ADRs).
- **Maintenance of the approved M1 planning documents** (corrections and review responses to [docs/m1-plan.md](docs/m1-plan.md) and ADRs 0014–0020) and of the checkpoint documentation ([HANDOFF.md](HANDOFF.md), per the checkpoint protocol below).
- **M1 implementation — Slice S3 only, effective only after its authorization PR merges to `main`.** S3 is limited to the scope and path boundary recorded in [TASKS.md](TASKS.md) and [HANDOFF.md § 7](HANDOFF.md): validated synthetic Evidence fixtures under `fixtures/demo-company/` (the seven approved scenarios and edge cases of [docs/m1-plan.md § 6](docs/m1-plan.md), parsing through the existing S1 schemas, pipeline inputs only), scenario documentation and a deterministic catalog/manifest, one narrowly scoped fixture-validation test under `packages/shared/src/`, and factual TASKS.md progress notes. **S3 does NOT authorize:** changes to existing S1 schemas or S2 contracts, repository/storage implementations, serialization/hashing, temporal or reconciliation algorithms, snapshot generation, API routes, frontend changes, connector work, or dependency/lockfile changes. Work is slice-driven and independently reviewed: **S4–S8 are unauthorized** until the preceding slice is reviewed, merged, and the next slice is explicitly released — do not start a later slice, even preparatorily, without that release being recorded in TASKS.md.
...
- Maintenance, corrections, and authorized M1 slice work are permitted, but only using the technologies the accepted ADRs name — dependencies beyond them still require justification and, if significant, a new ADR (Zod, named by accepted ADR-0005, was introduced in S1 with its justification-at-PR).
...
fixtures/            Synthetic test data (populated by M1 Slice S3 — authorized 2026-07-30, effective on merge of the authorization PR)
