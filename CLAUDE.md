...
- **No implementation slice is currently authorized.** Work is slice-driven and independently reviewed: **S8 remains unauthorized** until explicitly released — do not start it, even preparatorily, without that release being recorded in TASKS.md.
...
- **M1 Slice S7 (query API v1 routes) is complete** — implemented within [ADR-0024's Exact S7 Boundary](docs/adr/0024-m1-query-api-runtime-contract.md#exact-s7-boundary-restated-for-this-adrs-scope) as amended by ADR-0025, independently reviewed, and merged through PR #28 at `a7624cd` on 2026-08-11. Do NOT implement S8 — it remains gated on its own explicit release, not on S7's closure. Do NOT implement M2+ features (exploration UI, overlays, impact simulation, connectors) before their milestone is authorized.
