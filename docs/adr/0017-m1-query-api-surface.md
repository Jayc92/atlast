...
**Status:** Accepted; amended by [ADR-0020](0020-m1-inventory-and-search-semantics.md) and [ADR-0024](0024-m1-query-api-runtime-contract.md)
**Date:** 2026-07-23
...
> **Amendment notice (2026-08-11):** [ADR-0024](0024-m1-query-api-runtime-contract.md) (**Accepted** 2026-08-11, following the S7 pre-release architecture and contract review and three independent correction passes) **completes, without altering, the query-API specifics this ADR described only in general form**: the exact, complete HTTP route inventory (including three routes this ADR's seven-family table implies but does not name — no relationship-detail route, no relationship-scoped evidence-chain route, no assertion-revision route — and confirmation that `EvidenceStore.listEvidence` has no public HTTP route at all); the exact per-route path and query parameters, defaults, and pinning/pagination applicability; the exact HTTP query-string-to-read-mode coercion and partial-pin rejection rule; exact query-parameter wire names, numeric coercion, and unknown/repeated-key handling; the percent-encoding contract for multi-segment path-parameter identifiers; the exact snapshot-summary and Evidence-lookup response envelopes (resolving the mismatch between this ADR's general envelope and the repository's flat `SnapshotSummary`); a complete, closed external error-response contract mapping every repository error and request-validation failure to an exact HTTP status and typed `details` shape (settling the "single structured error schema" this ADR named without defining); cursor-transport policy at the HTTP boundary; the Zod-to-Fastify validation and mandatory response-validation mechanism; the asynchronous application-initialization lifecycle; the exact direct workspace dependencies `apps/api` requires; and the production-valid build/runtime package-entry-point strategy for `packages/shared`/`packages/graph-model`. This ADR's accepted decision text below is **preserved verbatim**; ADR-0024 (as amended, it in turn amends [ADR-0020](0020-m1-inventory-and-search-semantics.md) — see that ADR's own amendment notice) is the sole normative source for the specifics it adds. **ADR-0024's acceptance does not authorize M1 Slice S7 implementation** — S7 remains gated pending its own separate, explicit human release recorded in [TASKS.md](../../TASKS.md); no implementation slice is currently active.
