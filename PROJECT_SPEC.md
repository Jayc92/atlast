...
**Status:** Approved product baseline. M0 through M5 are complete; M5 formally closed at checkpoint `m5-complete` ([docs/milestones.md](docs/milestones.md), [TASKS.md](TASKS.md), [HANDOFF.md](HANDOFF.md)). M6 has an Accepted baseline — [docs/m6-plan.md](docs/m6-plan.md) plus ADR-0039, ADR-0040, and ADR-0041 (ADR-0042 remains Rejected). M6-A was separately, explicitly authorized and its implementation merged through [PR #100](https://github.com/Jayc92/atlast/pull/100) at `2288d6503675e03c1d635cd5de02ce64e44aac5e`; **M6-A is complete at checkpoint `m6-a-normal-website-kubernetes-merged`.** M6-A's completion does not complete M6: no M6 exit criterion is satisfied, and M6 remains open. No implementation slice is currently active. M6-B and M6-C remain gated and unauthorized, each requiring its own separate, explicit human authorization before any further implementation. This document itself authorizes no slice or milestone. Implementation details are governed by human-approved ADRs in [docs/adr/](docs/adr/README.md), which override any conflicting draft-era architectural wording remaining here; the vision, goals, principles, scope, and non-goals below remain the approved product baseline.
