...
The M0 foundation is complete — the backend API shell lives in [apps/api](apps/api) and the web application shell in [apps/web](apps/web) — and **M1 is active and slice-gated** ([TASKS.md](TASKS.md)). Slices S1–S7 are complete and merged (PRs #7, #10, #13, #16, #19, #23, and #28; S7's closeout checkpoint merged through PR #29). **Slice S7 (query API v1 routes in `apps/api`)** — its design was settled by [ADR-0024](docs/adr/0024-m1-query-api-runtime-contract.md) (accepted 2026-08-11 after the S7 pre-release review and three independent correction passes, amending ADR-0017/0020 via metadata-only notices), and Joseph Carfagno then explicitly authorized S7 on 2026-08-11. An actual S7-B implementation attempt reproduced a build-configuration contradiction, resolved by **[ADR-0025](docs/adr/0025-s7-source-alias-erasable-syntax-compatibility.md) (accepted 2026-08-11)** — the single-file `identity-normalization.ts` compatibility refactor. S7 was then implemented under accepted ADR-0024 as amended by ADR-0025, independently reviewed, merged to `main` through PR #28 at `a7624cd`, and formally closed through PR #29 at `9acfefa`, all on 2026-08-11. **Closing S7 did not authorize S8.** **Joseph Carfagno then explicitly authorized M1 Slice S8** (the M1 synthetic-boundary re-audit and documentation closeout) **in the conductor conversation on 2026-08-11**: S8 is the only active authorized implementation slice, and its authorization is now effective (authorization PR merged through PR #30 at `a4c6a5d` on 2026-08-12, `main` synchronized locally with a clean working tree). **Its implementation output exists as a release candidate, not yet independently reviewed, PR-approved, or merged** — it still requires implementation review, verification, PR approval, merge, and a final M1 closeout checkpoint, and **M1 must not be called complete until S8 closes**. Contributions are limited to the authorized S8 scope and to maintenance and corrections of the existing foundation and merged S1–S7 slices ([HANDOFF.md § 7](HANDOFF.md)). M2+ remain gated on their own explicit releases ([docs/milestones.md](docs/milestones.md), [HANDOFF.md](HANDOFF.md)). All contributions — documentation or code — must comply with [GUARDRAILS.md](GUARDRAILS.md).
