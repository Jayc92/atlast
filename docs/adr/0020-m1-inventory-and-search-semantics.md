...
**Status:** Accepted — amends ADR-0017; amended by [ADR-0024](0024-m1-query-api-runtime-contract.md)
**Date:** 2026-07-29
...
> **Amendment notice (2026-08-11):** [ADR-0024](0024-m1-query-api-runtime-contract.md) (**Accepted** 2026-08-11, following the S7 pre-release architecture and contract review and three independent correction passes) amends **exactly one** phrase in this ADR's § 1 ("Inventory: entity-only, with an optional claim-level `entityType` filter"): the sentence "No relationship-inventory family is added; Relationship subjects reach consumers through **entity detail and traversal**, exactly as ADR-0017 already defines" is corrected to **"search and traversal"**. Entity detail (`GET /api/v1/entities/{entityId}`) validates its path parameter as an Entity identifier before any repository call and therefore can never return a Relationship subject; the two routes that actually surface a Relationship subject are identifier search (`GET /api/v1/search`) and traversal. ADR-0024 § 1/§ "Relationship to Accepted ADRs" is the sole normative source for this correction and for why the resulting M1 surface (no dedicated relationship-detail or relationship-scoped evidence-chain route) still satisfies traceability. This ADR's decision text above is **preserved verbatim as accepted** — the sentence is quoted, not altered, in this notice; only the corrected reading is recorded here. **ADR-0024's acceptance does not authorize M1 Slice S7 implementation** — S7 remains gated pending its own separate, explicit human release recorded in [TASKS.md](../../TASKS.md); no implementation slice is currently active.
