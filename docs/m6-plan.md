...
- **M6-B (proposed shape, unauthorized):** the Kubernetes topology extension (ADR-0039: Deployments, ReplicaSets, Services, UID-based ownership matching, and the full six-state Service-relationship vocabulary), the minimum visual productization (§ 9), and the pilot-feedback artifact (ADR-0041).
- **M6-C (proposed shape, unauthorized; evidence/closeout slice — not primarily code):** build the deterministic pilot sandbox (§ 12), run the acceptance test (§ 14) with a real tester, record the scorecard, evaluate the exit criteria (§ 15) directly against the merged repository state, and prepare the milestone closeout.

This plan itself never grants authorization for any slice — each requires its own separate, explicit human authorization after its predecessor merges and `main` synchronizes cleanly (§ 22). M6-A has received and satisfied that requirement. M6-B and M6-C have not, and neither is authorized by M6-A's completion.
