...
- **Checkpoint name:** `m1-s8-authorized`
- **Latest merged checkpoint commit:** `9acfefa2fd56deab06f1740b68705e7b415fb5c9` (`docs: close M1 implementation slice S7 (#29)`) — the S7 closeout checkpoint, merged through [PR #29](https://github.com/Jayc92/atlast/pull/29) on 2026-08-11 with GitHub Actions `verify` passing.
- **Latest merged product commit:** `a7624cd6328c097dc4f752bc47a6a4863d36e968` (`feat: add M1 query API v1 (#28)`) — M1 Slice S7, squash-merged through [PR #28](https://github.com/Jayc92/atlast/pull/28) to `main` on 2026-08-11 with GitHub Actions `verify` passing in 2m14s. It remains the product state this checkpoint captures — S8 is authorized (below) but not yet implemented.
- **This document update records the explicit S8 authorization:** Joseph Carfagno explicitly authorized M1 Slice S8 in the conductor conversation on 2026-08-11 ([TASKS.md](TASKS.md) records the decision). **The authorization is effective only after this documentation PR merges to `main` and `main` is synchronized locally with a clean working tree**; the record lands through its own documentation PR whose squash-merge SHA is intentionally not predicted here — always read the actual tip from Git, not from this file. **No S8 implementation file may be created before that.**
- **Branch state at this checkpoint:** `docs/m1-s8-authorization`, based on merged `main` at `9acfefa`, clean working tree, no stash entries.
...
- **S8 is explicitly authorized and is the only active authorized implementation slice** — Joseph Carfagno made the separate, explicit S8 authorization decision in the conductor conversation on 2026-08-11, recorded in [TASKS.md](TASKS.md). **The authorization is effective only after the documentation PR recording it merges to `main` and `main` is synchronized locally with a clean working tree** — no S8 implementation file may be created before that. It **does not approve future S8 implementation output** — that output still requires implementation, independent review, passing local `./scripts/verify.sh` verification, PR approval with GitHub Actions `verify` passing, merge, and a final M1 closeout checkpoint. S8 is not implemented, reviewed, approved, complete, or merged. **M1 must not be called complete until S8 closes.**
...
| S8    | Acceptance additions (only if the shell changes), M1 boundary re-audit, documentation closeout                                                                                | Authorized 2026-08-11 (effective after this PR merges and `main` synchronizes) |
...
- **PR #29 (commit `9acfefa2fd56deab06f1740b68705e7b415fb5c9`) is the latest merged change** — the S7 closeout checkpoint documentation, squash-merged to `main` on 2026-08-11 with GitHub Actions `verify` passing. **S1–S7 are complete; S8 is authorized (below) but not yet implemented.** The S7 product commit merged through [PR #28](https://github.com/Jayc92/atlast/pull/28) at `a7624cd` on 2026-08-11 with GitHub Actions `verify` passing in 2m14s (immediately preceding); the ADR-0025 acceptance record merged through PR #27 at `2480aa3`; the S7 authorization documentation merged through PR #26 at `5a0eeca`; the ADR-0024 acceptance record merged through PR #25 at `4af2b6e`.
- **This S8 authorization documentation is the checkpoint record itself.** It lands through its own documentation PR; only when that PR merges does its merge commit become the latest merged checkpoint commit — and only then does the S8 authorization become effective. The tip of `main` will therefore move past `9acfefa`, and this artifact intentionally does not predict that PR's squash-merge SHA.
- At this checkpoint, `docs/m1-s8-authorization` is based on merged `main` at `9acfefa`, the working tree is clean, and no stash entries remain.
- No implementation branch is active yet; any open branches are documentation branches for this checkpoint. S8 implementation may begin only after this authorization PR merges and `main` is synchronized with a clean working tree.
...
**Slice S8 — M1 synthetic-boundary re-audit and documentation closeout — is the only active authorized implementation slice.** Joseph Carfagno **explicitly authorized S8 in the conductor conversation on 2026-08-11**, recorded in [TASKS.md](TASKS.md), separately from and after S7's formal closeout (merged through [PR #29](https://github.com/Jayc92/atlast/pull/29) at `9acfefa`). **The authorization is effective only after the documentation PR recording it merges to `main` and `main` is synchronized locally with a clean working tree** — no S8 implementation file may be created before that. **Authorization does not approve future S8 implementation output**: that output still requires implementation, independent review, complete local `./scripts/verify.sh` verification, PR review, successful GitHub Actions `verify`, merge, and a final M1 closeout checkpoint. S8 is not implemented, reviewed, approved, complete, or merged. **M1 must not be called complete until S8 closes.**
...
2. **Factual M1 documentation and checkpoint closeout** — updating [TASKS.md](TASKS.md), this document, [docs/milestones.md](docs/milestones.md), and [docs/m1-plan.md](docs/m1-plan.md) to record the audit result and the final M1 exit-criteria status.
...
**Permitted work now:** the authorized S8 scope above (effective only once this authorization PR merges and `main` is synchronized with a clean working tree); maintenance and corrections of the merged M0 foundation and the merged S1–S7 slices; maintenance of the approved planning and checkpoint documentation. Nothing else — no M2+ work of any kind, including "preparatory" implementation.

**Next-agent preflight** (run before acting on this checkpoint): `git status` (clean tree required), `git log --oneline --decorate -10` (confirm position against this document — the S8 authorization is effective only if the PR recording it has merged to `main`), then read [TASKS.md](TASKS.md) before any work. S8 is the only implementation slice with an explicit human release recorded in TASKS.md.
...
This checkpoint records condition 5 for S7's close: **S8 has now received its explicit human authorization**, recorded in [TASKS.md](TASKS.md). Conditions 1–4 for _this_ authorization record itself (the PR that carries it being merged, `main` synchronized with a clean working tree, verification recorded, and this document reflecting the merged state) must still be satisfied before the authorization becomes effective and any S8 implementation file may be created.

## 8. Prohibited Work

**S8 is the only authorized implementation slice** (effective only after this authorization PR merges and `main` is synchronized locally with a clean working tree — no S8 file may be created before that). Everything else remains prohibited, in particular:

- Any S8 implementation before the authorization PR recording this decision has merged to `main` and `main` is synchronized locally with a clean working tree.
...
- **Declaring M1 complete before S8 is implemented, reviewed, verified, merged, and formally closed.**
- **Frontend changes of any kind beyond the narrowly bounded shell-status exception in § 7 item 3 — and even that exception is not authorized by this task, only by S8's own future implementation.**
...
- Current slice state: S1–S7 are complete and merged (S7, the
  ...
  (recorded in TASKS.md): S8 is the only active authorized
  implementation slice, effective only after the documentation PR
  recording the authorization merged to main with main synchronized
  locally and a clean working tree — verify that merge in Git before
  treating S8 as releasable. The authorization does not approve S8
  implementation output, which still requires independent review,
  verification, PR approval, merge, and a final M1 closeout
