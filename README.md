...
The M0 foundation is complete — the backend API shell lives in [apps/api](apps/api) and the web application shell in [apps/web](apps/web) — and **M1 is active and slice-gated** ([TASKS.md](TASKS.md)). Slices S1–S5 are complete and merged (PRs #7, #10, #13, #16, and #19). **No implementation slice is currently active** — closing S5 did not authorize S6, and the next permitted action is an S6 pre-release architecture and authorization review, not S6 implementation. Contributions are limited to maintenance and corrections of the existing foundation and merged slices ([HANDOFF.md § 7](HANDOFF.md)). S6–S8 and M2+ remain gated on their own explicit releases ([docs/milestones.md](docs/milestones.md), [HANDOFF.md](HANDOFF.md)). All contributions — documentation or code — must comply with [GUARDRAILS.md](GUARDRAILS.md).
