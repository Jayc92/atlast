...
- **Checkpoint name:** `m1-s7-query-api-merged`
- **Latest merged product commit:** `a7624cd6328c097dc4f752bc47a6a4863d36e968` (`feat: add M1 query API v1 (#28)`) — M1 Slice S7, squash-merged through [PR #28](https://github.com/Jayc92/atlast/pull/28) to `main` on 2026-08-11 with GitHub Actions `verify` passing in 2m14s. It is the product state this checkpoint captures — **S1–S7 are now complete; no implementation slice is currently active.**
- **This document update records the S7 closeout.** The prior checkpoint (`m1-s7-authorized`) recorded S7's explicit authorization and a subsequently reproduced, temporary build-configuration block (resolved by accepted [ADR-0025](docs/adr/0025-s7-source-alias-erasable-syntax-compatibility.md)). That block is resolved and superseded: S7 was implemented under accepted ADR-0024 as amended by ADR-0025, independently reviewed, and merged through PR #28. This closeout documentation lands through its own documentation PR (branch `docs/m1-s7-closeout`); its own squash-merge SHA is intentionally not predicted here — always read the actual tip from Git, not from this file.
- **Branch state at this checkpoint:** `docs/m1-s7-closeout`, based on merged `main` at `a7624cd`, clean working tree, no stash entries.
...
- **S7 is complete** — Joseph Carfagno explicitly authorized S7 on 2026-08-11, recorded in [TASKS.md](TASKS.md). An actual implementation attempt reproduced a genuine, implementation-critical contradiction between accepted ADR-0011 (`apps/api`'s `erasableSyntaxOnly: true`) and accepted ADR-0024 § 14 step 5 (the `@atlast/graph-model` typecheck/test source alias); [ADR-0025](docs/adr/0025-s7-source-alias-erasable-syntax-compatibility.md), explicitly accepted by Joseph Carfagno on 2026-08-11, resolved it by authorizing exactly one narrow, behavior-preserving refactor to `packages/graph-model/src/identity-normalization.ts` (amending ADR-0022/0024 via metadata-only notices). S7 was then implemented under accepted ADR-0024 as amended by ADR-0025, independently reviewed, and **merged to `main` through [PR #28](https://github.com/Jayc92/atlast/pull/28) at `a7624cd` on 2026-08-11 with GitHub Actions `verify` passing in 2m14s.** **S1–S7 are now complete. No implementation slice is currently active — closing S7 does not authorize S8.**
- **S8 remains gated and unauthorized**, on its own separate, explicit release. No S8 closeout, frontend, connector, infrastructure, or dependency work is authorized. No frontend work is authorized; `apps/web` remains untouched.
...
- **PR #28 (commit `a7624cd6328c097dc4f752bc47a6a4863d36e968`) is the latest merged product change** — M1 Slice S7, squash-merged to `main` on 2026-08-11 with GitHub Actions `verify` passing in 2m14s. **S1–S7 are now complete.** The ADR-0025 acceptance record merged through PR #27 at `2480aa3` on 2026-08-11 (immediately preceding); the S7 authorization documentation merged through PR #26 at `5a0eeca`; the ADR-0024 acceptance record merged through PR #25 at `4af2b6e`; the S6 closeout through PR #23 at `9bf7f09` on 2026-08-10.
- **This S7 closeout documentation is itself a new documentation checkpoint.** It lands through its own documentation PR from branch `docs/m1-s7-closeout`, based on merged `main` at `a7624cd`; this artifact intentionally does not predict that PR's own squash-merge SHA.
- At this checkpoint, `docs/m1-s7-closeout` is based on merged `main` at `a7624cd`, the working tree is clean, and no stash entries remain.
- No implementation branch is active. S8 remains gated pending its own separate, explicit release.
...
**No implementation slice is currently active.** S1–S7 are complete and merged — S7 (query API v1 routes in `apps/api`) through [PR #28](https://github.com/Jayc92/atlast/pull/28) at `a7624cd` on 2026-08-11, implementing accepted [ADR-0024](docs/adr/0024-m1-query-api-runtime-contract.md) as amended by [ADR-0025](docs/adr/0025-s7-source-alias-erasable-syntax-compatibility.md). **Closing S7 does not authorize S8.** S8 (acceptance additions only if the shell changes, the M1 boundary re-audit, and documentation closeout) remains gated on its own separate, explicit human release recorded in [TASKS.md](TASKS.md).

**What S7 delivered** (details in [TASKS.md](TASKS.md), `apps/api/src/`, the additive `packages/shared/src/**` HTTP contracts, and the one authorized `packages/graph-model/src/identity-normalization.ts` refactor), implementing accepted ADR-0024 as amended by ADR-0025:

- The additive `packages/shared` HTTP contracts: route 7's narrowed snapshot-summary envelope, route 5's Evidence-lookup envelope, the closed discriminated `errorResponseSchema` with its cursor-mismatch/unknown-identifier/invalid-read-coordinate detail shapes, and two HTTP query-coercion helper schemas — no existing S1/S2 schema, type, or interface changed.
- The production-valid package-entry-point build plumbing for `packages/shared`/`packages/graph-model` (real `dist`/`main`/`types`/`exports`), with the typecheck/test-only source-alias convention retained by design.
- The one authorized behavior-preserving refactor to `packages/graph-model/src/identity-normalization.ts` (ADR-0025): `IdentityNormalizationError`'s constructor-parameter properties replaced with `declare readonly` fields plus explicit constructor assignment, resolving the `erasableSyntaxOnly`/source-alias contradiction that an actual implementation attempt reproduced — no other line, export, or file in either package touched.
- In `apps/api`: request/query coercion (partial-pin rejection identical with or without a cursor, wire-name mapping, strict numeric coercion); the closed and unconditionally redacted error-response contract, including a `mapFrameworkError` dispatch distinguishing genuinely malformed client input (`FST_ERR_BAD_URL`, `FST_ERR_MAX_PARAM_LENGTH`) from internal Fastify failures (e.g. `FST_ERR_ASYNC_CONSTRAINT`), which redact to `INTERNAL_ERROR`; mandatory response-schema validation before every successful send; all seven accepted query routes (entity inventory, entity detail, identifier search, bounded traversal, Evidence lookup, the entity-scoped evidence chain, and pinned snapshot summary) plus the unchanged `GET /health`; and an asynchronous, fixture-backed `initializeApplication`/`buildApplication` composition root guaranteeing no request can be served before fixture ingestion completes.
- Comprehensive colocated `fastify.inject()` integration tests covering every route's success path, latest/pinned reproducibility, cursor continuation and binding mismatch, every closed error code (including response-schema-violation and framework-error redaction proofs via deterministic stubs), and route-not-found handling.

**S7 stayed within its authorized scope**: no change to any existing S1/S2 domain or repository schema/interface, `packages/graph-model`'s or `packages/shared`'s existing `src/**` runtime behavior beyond the additive schemas and the one named refactor, the S6 cursor payload shape or its binding semantics, `fixtures/demo-company/**`, `apps/web`, `scripts/verify.sh`, `scripts/bootstrap.sh`, `pnpm-workspace.yaml`, or any S8/M2+ work.

**Final S7 verification:** 64 `apps/api` tests passing in 7 files (shared 373/373 in 14 files, graph-model 372/372 in 19 files, web 5/5 — all unchanged); the complete unmodified `./scripts/verify.sh` passed all seven stages locally (including a clean-build-then-run proof and browser acceptance 2/2), and GitHub Actions `verify` passed on PR #28 in 2m14s.

**Permitted work now:** maintenance and corrections of the merged M0 foundation and the merged S1–S7 slices; maintenance of the approved planning and checkpoint documentation. Nothing else — no S8 or M2+ work of any kind, including "preparatory" implementation, until S8 receives its own explicit human release.

**Next-agent preflight** (run before acting on this checkpoint): `git status` (clean tree required), `git log --oneline --decorate -10` (confirm position against this document), then read [TASKS.md](TASKS.md) before any work. No slice currently has an explicit human release recorded in TASKS.md; S8 has none.
...
This checkpoint records conditions 1–4 for S7's close (PR #28 merged, verification recorded above, this document reflecting the merged state). **Condition 5 for the next slice (S8) has not been given** — S8 remains gated until Joseph Carfagno explicitly releases it, recorded in TASKS.md.

## 8. Prohibited Work

**No implementation slice is currently authorized.** In particular:

- Any S8 work (acceptance additions, M1 boundary re-audit, documentation closeout) before its own separate, explicit release.
- Any change to existing S1–S7 behavior beyond maintenance corrections — including any existing S1/S2 domain or repository schema/interface, the merged S3 fixture catalog, the merged S4 temporal foundations, the merged S5 reconciliation engine, the merged S6 snapshot/storage layer, or the merged S7 query API routes, error contract, or composition root.
...
- Current slice state: S1–S7 are complete and merged (S7, the
  ...
  accepted ADR-0024 as amended by ADR-0025). No implementation
  slice is currently active — closing S7 does not authorize S8.
...
- **Implementation attempts keep surfacing gaps design review alone did not catch — a pattern, not a one-off.** The ADR-0022/0023/0024/0025 lineage found four such gaps in succession (most recently the `erasableSyntaxOnly`/source-alias contradiction ADR-0025 resolved, and, within S7-B itself, a Fastify `frameworkErrors`-routing gap for `FST_ERR_ASYNC_CONSTRAINT` found and fixed during S7's own review-remediation pass). All are now resolved and merged; the mitigation going forward is unchanged — treat every slice's implementation attempt as part of this project's review process, not merely its execution, and budget review time accordingly for S8 and beyond.
