...
- **No M1 implementation slice is currently released.** S3 (fixture suite in `fixtures/demo-company/`) is next but remains gated on its own explicit human release recorded in TASKS.md. Work is slice-driven and independently reviewed: **S3–S8 are unauthorized** until the preceding slice is reviewed, merged, and the next slice is explicitly released — do not start a later slice, even preparatorily, without that release being recorded in TASKS.md.
