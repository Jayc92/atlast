...
- **Maintenance of the approved M1 planning documents** (corrections and review responses to [docs/m1-plan.md](docs/m1-plan.md) and ADRs 0014–0023) and of the checkpoint documentation ([HANDOFF.md](HANDOFF.md), per the checkpoint protocol below).
- **Slice S6 is the only active authorized implementation slice** — explicitly authorized by Joseph Carfagno on 2026-08-05 (recorded in [TASKS.md](TASKS.md)), **effective only after the documentation PR recording the authorization merges to `main` and `main` is synchronized locally with a clean working tree** — no S6 implementation file may be created before that. The authorized S6 scope is exactly [ADR-0023 § 10](docs/adr/0023-m1-snapshot-and-in-memory-store-semantics.md): `packages/graph-model/src/**` (snapshot construction; snapshot identity, payload, and checksum construction; the in-memory `EvidenceStore`/`TopologyGraphStore` implementations; the injected `Clock` type and derivation-version lookup; graph/Evidence cursor binding and continuation validation; identity-scoped referential-integrity enforcement; the ADR-0023 error taxonomy; contract-suite registration via an injected fixture loader; colocated tests) plus TASKS.md for factual progress notes — no `packages/shared`, fixture, manifest, lockfile, dependency, `apps/api`, `apps/web`, script, `m1-v2`, or S7+ change. The authorization does not approve future S6 implementation output — that output still requires independent review, verification, PR approval, and merge. Work is slice-driven and independently reviewed: **S7–S8 remain unauthorized** until each is explicitly released — do not start a later slice, even preparatorily, without that release being recorded in TASKS.md.
...
- Do NOT implement any M1 slice other than S6 — S6 is the only released slice (explicitly authorized 2026-08-05, effective only after the authorization PR merges and `main` is synchronized; S7–S8 remain gated on their own explicit releases) — and do NOT implement M2+ features (exploration UI, overlays, impact simulation, connectors) before their milestone is authorized.
