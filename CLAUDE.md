...
**M0, M1, and M2 are complete. M3-A is complete.** The M3-A closeout merged through PR #56 at `a767c93` on 2026-08-16. Joseph Carfagno then explicitly authorized **M3-B only** on 2026-08-16. That release becomes operational only after this authorization record merges and local `main` is synchronized cleanly. **M3-C through M3-F and M4+ remain unauthorized.** See [TASKS.md](TASKS.md) and [HANDOFF.md](HANDOFF.md).
...
- **M3-B overlay-provider and deterministic-projection work**, strictly limited to the internal `packages/overlay-model` workspace package, its in-memory `OperationalOverlayStore` implementation, its pure projector, exhaustive corresponding tests, directly required workspace/build plumbing, and factual `TASKS.md` measurements. No API route, browser behavior, external dependency, topology mutation, or M3-C+ behavior is authorized. This permission becomes operational only after the authorization record merges and `main` is synchronized cleanly.

**M3-B is the only authorized implementation slice, pending the documentation merge gate. M3-C and every later slice or milestone remain unauthorized**, each gated on an explicit human release.
...
**M0, M1, and M2 are complete. The M3 baseline is accepted, M3-A is complete, and M3-B alone is separately authorized pending activation:**
...
- **M1 and M2 are formally complete, and M3-A is merged and closed.** M3-B may begin only after its authorization record merges and `main` is synchronized cleanly, and only within the exact boundary above and in [docs/m3-plan.md](docs/m3-plan.md). Do NOT implement M3-C+ or M4+ work before separate authorization.
