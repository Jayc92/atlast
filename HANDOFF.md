...
- **Checkpoint name:** `m1-s7-authorized`
- **Latest merged checkpoint commit:** `4af2b6e93cdc44e9969d0fc7e5df9d6987b245ab` (`docs: accept ADR-0024 query API runtime contract (#25)`) — the ADR-0024 acceptance record, merged through [PR #25](https://github.com/Jayc92/atlast/pull/25) on 2026-08-11 with GitHub Actions `verify` passing.
- **Latest merged product commit:** `9bf7f09ea2bd9c755f76dc09710a3bb5e0bf715c` (`feat: add M1 snapshot and in-memory repositories (#23)`) — M1 Slice S6, squash-merged through [PR #23](https://github.com/Jayc92/atlast/pull/23) to `main` on 2026-08-10 with GitHub Actions `verify` passing in 2m12s. It remains the product state this checkpoint captures — S7 is authorized (below) but not yet implemented.
- **This document update records the explicit S7 authorization:** Joseph Carfagno explicitly authorized M1 Slice S7 on 2026-08-11 ([TASKS.md](TASKS.md) records the decision), separately from and after his acceptance of [ADR-0024](docs/adr/0024-m1-query-api-runtime-contract.md) — the binding S7 runtime contract — on 2026-08-11. **The authorization is effective only after this documentation PR merges to `main` and `main` is synchronized locally with a clean working tree**; the record lands through its own documentation PR whose squash-merge SHA is intentionally not predicted here — always read the actual tip from Git, not from this file. **No S7 implementation branch or file may be created before that.**
...
- **S7 is explicitly authorized and is the only active authorized implementation slice** — Joseph Carfagno made the separate, explicit S7 authorization decision on 2026-08-11, recorded in [TASKS.md](TASKS.md). **The authorization is effective only after the documentation PR recording it merges to `main` and `main` is synchronized locally with a clean working tree** — no S7 implementation file may be created before that. It **does not approve future S7 implementation output** — that output still requires implementation under accepted ADR-0024, independent review, passing local `./scripts/verify.sh` verification, PR approval with GitHub Actions `verify` passing, and merge. S7 is not implemented, reviewed, approved, complete, or merged.
- **S7 is currently temporarily blocked.** An actual M1 Slice S7-B implementation attempt reproduced a genuine, implementation-critical contradiction between accepted ADR-0011 (`apps/api`'s `erasableSyntaxOnly: true`) and accepted ADR-0024 § 14 step 5 (the `@atlast/graph-model` typecheck/test source alias) — see [TASKS.md](TASKS.md) and [ADR-0025](docs/adr/0025-s7-source-alias-erasable-syntax-compatibility.md) for the full finding and fix. **ADR-0025 was explicitly accepted by Joseph Carfagno on 2026-08-11**, amending ADR-0022 and ADR-0024 via metadata-only notices. **Acceptance authorizes exactly the named `identity-normalization.ts` refactor and a factual TASKS.md update — no second, separate release**, since S7 is already authorized and active and this ADR only widens its exact boundary for this one prerequisite fix; **acceptance does not approve S7-A/S7-B output and does not authorize S8/M2+**. **The authorized refactor is not yet implemented.** **Slice S7-A** was independently reviewed with no blocking findings and is locally checkpoint-committed at `747a32a` on branch `feat/m1-s7-query-api` in the primary implementation working tree — not pushed, not PR-approved, and not merged; **Slice S7-B is uncommitted**, paused at this blocker. Neither S7-A nor S7-B is pushed, PR-approved, or merged. S7 remains the only active authorized implementation slice, but implementation cannot proceed past this point until this acceptance record merges to `main`, the primary S7 branch synchronizes onto that checkpoint, and the refactor is implemented and verified.
- **S8 remains gated and unauthorized**, on S7's merge and its own explicit release. No S8 closeout, frontend, connector, infrastructure, or dependency work is authorized. No frontend work is authorized; `apps/web` remains untouched. **S7 is the only active authorized implementation slice; S7 is temporarily blocked only until [ADR-0025](docs/adr/0025-s7-source-alias-erasable-syntax-compatibility.md)'s acceptance record merges to `main`, the primary S7 branch is synchronized onto that checkpoint, and the authorized narrow refactor is implemented and verified. S8 remains gated and unauthorized.**
...
| S7    | Query API v1 routes in `apps/api` with integration tests as executable specification                                                                                          | Authorized 2026-08-11 (effective after this PR merges and `main` synchronizes) |
...
- **PR #23 (commit `9bf7f09ea2bd9c755f76dc09710a3bb5e0bf715c`) is the latest merged product change** — M1 Slice S6, squash-merged to `main` on 2026-08-10 with GitHub Actions `verify` passing in 2m12s. It remains the product state this checkpoint captures — S7 is authorized (below) but not yet implemented. The ADR-0024 acceptance record merged through PR #25 at `4af2b6e` on 2026-08-11 (the latest merged checkpoint commit); the ADR-0023 acceptance record merged through PR #21 at `70df8c2`; the S6 authorization documentation merged through PR #22 at `dc55363` on 2026-08-06 (historical context only); S5's closeout checkpoint through PR #20 at `afb2359`; S5's authorization packet through PR #18 at `f50f0d7`.
- **This S7 authorization documentation is the checkpoint record itself.** It lands through its own documentation PR; only when that PR merges does its merge commit become the latest merged checkpoint commit — and only then does the S7 authorization become effective. The tip of `main` will therefore move past `4af2b6e`, and this artifact intentionally does not predict that PR's squash-merge SHA.
- At the checkpoint, `main` is synchronized with `origin/main` and the working tree is clean.
- No implementation branch is active yet; any open branches are documentation branches for this checkpoint. S7 implementation may begin only after this authorization PR merges and `main` is synchronized with a clean working tree.
...
**Slice S7 — Query API v1 routes in `apps/api` — is the only active authorized implementation slice.** Joseph Carfagno **explicitly authorized S7 on 2026-08-11** in a separate authorization decision recorded in [TASKS.md](TASKS.md), following — and distinct from — his acceptance of [ADR-0024](docs/adr/0024-m1-query-api-runtime-contract.md) the same day. **The authorization is effective only after the documentation PR recording it merges to `main` and `main` is synchronized locally with a clean working tree** — no S7 implementation file may be created before that. **Authorization does not approve future S7 implementation output**: that output still requires implementation under accepted ADR-0024, independent review, complete local `./scripts/verify.sh` verification, PR review, successful GitHub Actions `verify`, and merge. S7 is not implemented, reviewed, approved, complete, or merged.

**S7 is currently temporarily blocked on a reproduced build-configuration contradiction.** An actual S7-B implementation attempt found that `apps/api`'s composition root cannot both satisfy accepted ADR-0011 (`erasableSyntaxOnly: true`) and actually import from `@atlast/graph-model` through the accepted ADR-0024 § 14 step 5 source alias, because `packages/graph-model/src/identity-normalization.ts`'s `IdentityNormalizationError` constructor uses two TypeScript parameter properties — non-erasable syntax that TypeScript checks under `apps/api`'s own stricter compiler options once the alias is actually exercised (`TS1294`). This is documented in [ADR-0025](docs/adr/0025-s7-source-alias-erasable-syntax-compatibility.md), proposing exactly one narrow, behavior-preserving refactor (parameter properties → `declare readonly` fields with explicit constructor assignment, in the exact order the original emit already produces). **ADR-0025 was explicitly accepted by Joseph Carfagno on 2026-08-11**, amending ADR-0022 and ADR-0024 via metadata-only notices. Acceptance authorizes exactly the named refactor and a factual TASKS.md update — no second, separate implementation release, since S7 is already authorized and active and this ADR only widens S7's own exact boundary for this one prerequisite fix; acceptance does not approve S7-A/S7-B implementation output and does not authorize S8/M2+. **The authorized refactor is not yet implemented.** Recorded honestly: **Slice S7-A** (build plumbing and additive shared HTTP schemas) was independently reviewed with no blocking findings and is locally checkpoint-committed at `747a32a` on branch `feat/m1-s7-query-api` in the primary implementation working tree — not pushed, not PR-approved, and not merged; **Slice S7-B** — currently uncommitted HTTP support modules, all seven route modules, the `app.ts`/`server.ts` composition-root rewrites, and the rewritten health test, with **comprehensive integration tests not yet added** — is **uncommitted**, paused exactly at this blocker before any integration-test file was written. **No S7-A or S7-B output is pushed, PR-approved, or merged.** S7 remains the only active authorized implementation slice, but cannot proceed past the composition root until this acceptance record merges to `main`, the primary S7 branch synchronizes onto that checkpoint, and the refactor is implemented and verified.
...
**The S7 design baseline:** [ADR-0024](docs/adr/0024-m1-query-api-runtime-contract.md) — the S7 pre-release review closing fifteen HTTP-boundary and build-boundary gaps left open by accepted ADR-0017 (as amended by ADR-0020) — was **explicitly accepted by Joseph Carfagno on 2026-08-11 after the S7 pre-release review and three independent correction passes**: the exact route inventory and per-route parameters, request/query coercion, the closed error-response contract, cursor-transport policy, response validation, asynchronous application initialization, the direct workspace dependencies `apps/api` requires, and the production-valid build strategy for `packages/shared`/`packages/graph-model`. With acceptance, ADR-0024 amends ADR-0017 and ADR-0020 via metadata-only notices (their accepted decision text preserved verbatim). **ADR-0024 is the binding S7 runtime contract and remains Accepted, unchanged** — design acceptance and implementation authorization (both 2026-08-11, but separate decisions) are distinct gates, both now passed for S7.

**Authorized S7 implementation scope** (per [ADR-0024's Exact S7 Boundary](docs/adr/0024-m1-query-api-runtime-contract.md#exact-s7-boundary-restated-for-this-adrs-scope)):

- `apps/api/src/**` — `GET /health` plus exactly the seven accepted query routes; request/query coercion; the closed error mapping; response validation before every successful send; the asynchronous composition root; and colocated `fastify.inject()` integration tests.
- `apps/api/tsconfig.json` — only the two new workspace source-alias `paths` entries (`@atlast/graph-model`, `@atlast/shared`), for typecheck/test only.
- `apps/api/tsconfig.build.json` — only the build-time empty `"paths": {}` override.
- `apps/api/vitest.config.ts` (new file) — only the two matching `resolve.alias` entries.
- `apps/api/package.json` — direct workspace dependencies on `@atlast/graph-model` and `@atlast/shared`; **no new third-party runtime dependency**.
...
- `packages/shared/tsconfig.build.json` (new) and `package.json` — only ADR-0024's build plumbing (`declaration`, `rewriteRelativeImportExtensions`, the `build` script, `main`/`types`/`exports`).
- `packages/graph-model/tsconfig.build.json` (new) and `package.json` — only ADR-0024's identical build plumbing (including its `"paths": {}` and `rewriteRelativeImportExtensions` settings); `packages/graph-model`'s existing `tsconfig.json` and `vitest.config.ts` are **not** touched; **existing `packages/graph-model` source runtime behavior remains frozen**.
- `TASKS.md` — solely for factual S7 progress reporting.

**S7 must NOT implement or modify:** a relationship-detail route; a relationship-scoped evidence-chain route; a bulk Evidence-listing route; an assertion route; any existing S1/S2 domain or repository schema, type, or interface; `packages/graph-model`'s or `packages/shared`'s existing `src/**` runtime behavior beyond the additive schemas named above; the S6 cursor payload shape or its binding semantics; `fixtures/demo-company/**`; `apps/web` (**no frontend work is authorized; `apps/web` remains untouched**); authentication, connectors, deployment, infrastructure, or real-system access; any new third-party dependency or version upgrade; `scripts/verify.sh` or `scripts/bootstrap.sh`; `pnpm-workspace.yaml`; or any S8 or M2+ work.

**Permitted work now:** the authorized S7 implementation scope above (effective only once this authorization PR merges and `main` is synchronized with a clean working tree); maintenance and corrections of the merged M0 foundation, S1–S6 slices, and accepted ADR-0024 design; maintenance of the approved planning and checkpoint documentation. Nothing else — no S8 or M2+ work of any kind, including "preparatory" implementation.

**Next-agent preflight** (run before acting on this checkpoint): `git status` (clean tree required), `git log --oneline --decorate -10` (confirm position against this document — the S7 authorization is effective only if the PR recording it has merged to `main`), then read [TASKS.md](TASKS.md) before any work. S7 is the only implementation slice with an explicit human release recorded in TASKS.md; S8 has none.
...
This checkpoint records condition 5 for S6's close: **S7 has now received its explicit human authorization**, recorded in [TASKS.md](TASKS.md). Conditions 1–4 for _this_ authorization record itself (the PR that carries it being merged, `main` synchronized with a clean working tree, verification recorded, and this document reflecting the merged state) must still be satisfied before the authorization becomes effective and any S7 implementation file may be created.

## 8. Prohibited Work

**S7 is the only authorized implementation slice** (effective only after this authorization PR merges and `main` is synchronized locally with a clean working tree — no S7 file may be created before that). Everything else remains prohibited, in particular:

- Any S7 implementation before the authorization PR recording this decision has merged to `main` and `main` is synchronized locally with a clean working tree.
- Any S7 work outside the authorized scope in § 7 — including a relationship-detail route, a relationship-scoped evidence-chain route, a bulk Evidence-listing route, an assertion route, any change to existing S1/S2 domain or repository schemas/interfaces, any change to `packages/graph-model`'s or `packages/shared`'s existing `src/**` runtime behavior beyond the additive schemas ADR-0024 names, any change to the S6 cursor payload shape or its binding semantics, `fixtures/demo-company/**`, `scripts/verify.sh`, `scripts/bootstrap.sh`, or `pnpm-workspace.yaml`.
- Any S8 work (acceptance additions, M1 boundary re-audit, documentation closeout) before its own separate, explicit release.
...
- **Frontend changes of any kind — no frontend work is authorized; `apps/web` remains untouched.**
- Authentication, connectors, deployment, infrastructure, or real-system access.
- New third-party dependencies, version upgrades, or lockfile changes beyond the deterministic `pnpm-lock.yaml` importer-block consequence of the two authorized workspace dependency declarations.
...
- Current slice state: S1–S6 are complete and merged (S6, the
  ...
  metadata-only notices) and remains Accepted, unchanged. Separately
  from that acceptance, Joseph Carfagno explicitly authorized Slice
  S7 on 2026-08-11 (recorded in TASKS.md): S7 is the only active
  ...
  merge in Git before treating S7 as releasable. The authorization
  does not approve S7 implementation output, which still requires
  independent review, verification, PR approval, and merge.
  S8 and M2+ remain gated, each on its own explicit human
...
- **S7 is currently temporarily blocked on a build-configuration contradiction implementation surfaced, not design review.** `apps/api`'s existing `erasableSyntaxOnly: true` (ADR-0011) and the ADR-0024 § 14 step 5 source alias for `@atlast/graph-model` are individually sound but jointly incompatible the moment the alias is actually exercised by an import, because of two parameter properties in one already-merged S5 file (`identity-normalization.ts`). [ADR-0025](docs/adr/0025-s7-source-alias-erasable-syntax-compatibility.md) documents the finding and a narrow fix, and was **explicitly accepted by Joseph Carfagno on 2026-08-11**; **the authorized refactor is not yet implemented**, and S7 remains blocked until this acceptance record merges to `main`, the primary S7 branch synchronizes onto that checkpoint, and the refactor is implemented and verified. This is the fourth gap in the ADR-0022/0023/0024/0025 lineage found only by real implementation, reinforcing that implementation attempts remain part of this project's review process, not merely its execution.
