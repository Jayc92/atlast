...
The M0 foundation is complete — the backend API shell lives in [apps/api](apps/api) and the web application shell in [apps/web](apps/web) — and **M1 is active and slice-gated** ([TASKS.md](TASKS.md)). Contributions may include maintenance and corrections of the existing foundation and the currently authorized **Slice S2** scope (repository interfaces and the storage-agnostic contract-test suite skeleton in `packages/shared`); S3–S8 and M2+ remain gated on their own explicit releases ([docs/milestones.md](docs/milestones.md)). All contributions — documentation or code — must comply with [GUARDRAILS.md](GUARDRAILS.md).
