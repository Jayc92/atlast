...
- **M1 implementation — Slice S2 only** (S1 is complete, merged via PR #7 on 2026-07-29). S2 covers: async repository interfaces in `packages/shared` carrying the full M1 repository contract — reads pinned by (asOf, horizon, derivationVersion), bounded collection reads and traversal, Evidence lookup and evidence-chain contracts, subject and GraphAssertion revision reads, conflict and ambiguity retrieval, and the relationship endpoint referential-integrity obligations (defined in S2 per [ADR-0019](docs/adr/0019-subject-identity-and-assertion-claims.md) § 4, proven by the implementing slice) — plus a reusable, storage-agnostic contract-test suite skeleton. **S2 does NOT authorize:** any in-memory or database repository implementation (S6), fixture creation (S3), canonical serialization or hashing (S4/S6), temporal algorithms (S4), reconciliation (S5), API routes (S7), frontend changes, or any S3–S8 or M2+ work. Work is slice-driven and independently reviewed: **S3–S8 are unauthorized** until the preceding slice is reviewed, merged, and the next slice is explicitly released — do not start a later slice, even preparatorily, without that release being recorded in TASKS.md.
