# Atlast M6 Plan — Internal Pilot Validation (Accepted)

**Status:** Accepted. This document, and ADR-0039/0040/0041, are the accepted M6 architecture baseline. Baseline acceptance alone authorizes no implementation — each slice requires its own separate, explicit human authorization (§ 22). **M6-A has received that authorization, been implemented, independently reviewed and corrected, and merged** through [PR #100](https://github.com/Jayc92/atlast/pull/100), with its checkpoint documentation closed through [PR #101](https://github.com/Jayc92/atlast/pull/101). **M6-B has likewise received that authorization, been implemented, independently and adversarially reviewed twice, and merged** through [PR #104](https://github.com/Jayc92/atlast/pull/104), with its checkpoint documentation closed through [PR #105](https://github.com/Jayc92/atlast/pull/105) — including the separately Accepted [ADR-0043](adr/0043-m6-kubernetes-cross-kind-source-native-identity.md) cross-kind identity correction, merged through [PR #103](https://github.com/Jayc92/atlast/pull/103) before M6-B's corrective continuation. **M6-C has not** — it still requires its own separate, explicit human authorization before any implementation may begin.

> **Acceptance note (2026-08-24, historical):** Joseph Carfagno explicitly accepted the complete M6 baseline after both an adversarial architecture/product/security review and a genuinely independent final review found no substantive architecture, product, or security blocker (one wording-precision correction was applied to § 20 before acceptance). **The accepted architecture set is exactly this plan plus ADR-0039, ADR-0040, and ADR-0041.** [ADR-0042](adr/0042-m6-self-service-connector-launch-and-credential-model.md) remains **Rejected** and is **not** part of the accepted architecture set. **This acceptance authorized NO implementation of any kind on its own.** At the moment of acceptance, neither M6-A nor any later slice was yet authorized; implementation still required, in order: (1) this acceptance record to be committed to the reviewing PR; (2) that PR to be merged by Joseph Carfagno; (3) local `main` to synchronize cleanly with `origin/main`; (4) a separate, explicit human authorization of M6-A specifically. **All four of those steps have since occurred for M6-A; see § 22 for the current, complete gate record.**

> **Planning authorization boundary (historical):** Joseph Carfagno explicitly authorized M6 planning and ADR drafting only, on 2026-08-24, following the internal-product-validation reconnaissance recorded in this session's conversation history (no separate audit-document record exists for that reconnaissance; it was read-only and produced no file changes). This authorization released M6 planning and pre-release architecture/ADR review only, preceding the acceptance recorded immediately above — exactly as the M1 plan (ADRs 0014–0018), the M4 plan (ADRs 0032–0035), and the M5 plan (ADRs 0036–0037) each preceded their own first implementation slice.

> **Adversarial review note (2026-08-24, same day):** an independent adversarial architecture/product/security review of the first draft found material corrections required in the Kubernetes topology/ownership semantics, the Service-relationship vocabulary, the composition-root dataset model and connector lifecycle, the pilot-feedback verdict vocabulary, and the exact hypothetical-change contract (this plan's first draft loosely said "describe" a change, but the accepted contract supports only three predefined `changeType` values, never free text). It also found that the originally drafted ADR-0042 (self-service connect/launch) did not meet this project's own bar for a standalone ADR and folded its content into § 8 below. Every correction is reflected directly in the sections below and in ADRs 0039–0041; this note is not a duplicate changelog. This adversarial pass, followed by a genuinely independent final review (which required one further wording correction to § 20, since applied), is the review this plan's baseline was accepted on the strength of — see the acceptance note above.
...
- **M6-A — complete:** composition-root unification with the visible dataset-mode indicator and connector-failure lifecycle (ADR-0040), and the self-service connect/scan flow (§ 8, folded from the retracted ADR-0042), keeping the Kubernetes topology scope at M5's existing Pods-only coverage. This alone proved the single largest blocker — the browser could not see connector data — resolved, independently of any topology expansion. Separately authorized 2026-08-24, implemented, independently reviewed and corrected, merged through [PR #100](https://github.com/Jayc92/atlast/pull/100), and checkpointed through [PR #101](https://github.com/Jayc92/atlast/pull/101) at checkpoint `m6-a-normal-website-kubernetes-merged` (§ 22).
- **M6-B — complete:** the Kubernetes topology extension (ADR-0039: Deployments, ReplicaSets, Services, UID-based ownership matching, and the full six-state Service-relationship vocabulary), the minimum visual productization (§ 9), and the pilot-feedback artifact (ADR-0041). During implementation, a real cross-kind Kubernetes identity-collision defect was found and separately resolved through Accepted [ADR-0043](adr/0043-m6-kubernetes-cross-kind-source-native-identity.md) (merged through [PR #103](https://github.com/Jayc92/atlast/pull/103) before the corrective continuation). Separately authorized, implemented, independently and adversarially reviewed twice, merged through [PR #104](https://github.com/Jayc92/atlast/pull/104), and checkpointed through [PR #105](https://github.com/Jayc92/atlast/pull/105) at checkpoint `m6-b-kubernetes-topology-validation-merged` (§ 22).
- **M6-C (proposed shape, unauthorized; evidence/closeout slice — not primarily code):** build the deterministic pilot sandbox (§ 12), run the acceptance test (§ 14) with a real tester, record the scorecard, evaluate the exit criteria (§ 15) directly against the merged repository state, and prepare the milestone closeout. The two pre-M6-C readiness gates this status note previously named as open — (1) `scripts/connect-kubernetes-pilot.sh`'s Ctrl+C/TERM shutdown not stopping its launched API/web child processes; (2) the pilot-feedback panel discarding unexported session-local judgments if closed before export — were subsequently and separately authorized, implemented, independently and adversarially reviewed, and merged through [PR #107](https://github.com/Jayc92/atlast/pull/107) at checkpoint `m6-c-readiness-complete`; **both now evaluate PASS**. This closure does not itself authorize M6-C — M6-C still requires its own separate, explicit human authorization before any implementation, exactly as every prior milestone's next slice required.

This plan itself never grants authorization for any slice — each requires its own separate, explicit human authorization after its predecessor merges and `main` synchronizes cleanly (§ 22). M6-A and M6-B have received and satisfied that requirement. M6-C has not, and is not authorized by M6-B's completion.
...
Before M6-A could begin implementation, this plan required exactly five gates:

1. this plan and ADRs 0039–0041 receive independent architecture review — **complete**: both an adversarial architecture/product/security pass and a genuinely independent final review found no substantive blocker (one wording-precision correction to § 20 was applied before acceptance);
2. every blocking finding is corrected and re-reviewed — **complete**: none were found; the one required wording correction was applied and re-verified;
3. Joseph Carfagno explicitly accepts the complete M6 baseline (this plan and ADRs 0039–0041) — **complete, 2026-08-24**;
4. the acceptance record merges to `main` and local `main` is synchronized cleanly — **complete**: merged through [PR #98](https://github.com/Jayc92/atlast/pull/98), local `main` synchronized cleanly;
5. M6-A receives a separate, explicit implementation authorization, effective only once the record for gate 4 above has merged to `main` and local `main` is synchronized cleanly — **complete, 2026-08-24**: Joseph Carfagno separately and explicitly authorized M6-A specifically.

**All five gates required for M6-A were satisfied as of 2026-08-24.** This plan and its accompanying ADRs (0039/0040/0041) are **Accepted**, not merely Proposed. **M6-A was subsequently implemented, independently reviewed and corrected, and merged through [PR #100](https://github.com/Jayc92/atlast/pull/100) at `2288d6503675e03c1d635cd5de02ce64e44aac5e`; its checkpoint documentation closed through [PR #101](https://github.com/Jayc92/atlast/pull/101) at checkpoint `m6-a-normal-website-kubernetes-merged`.** Satisfying these five gates authorized, and this checkpoint's evidence closes, **M6-A only** — it does not authorize M6-B or M6-C. Each later slice requires this exact same five-gate sequence run again for its own scope, beginning with its own separate, explicit human authorization; **no implementation slice is currently active.**
