...
- **M6-C (proposed shape, unauthorized; evidence/closeout slice — not primarily code):** build the deterministic pilot sandbox (§ 12), run the acceptance test (§ 14) with a real tester, record the scorecard, evaluate the exit criteria (§ 15) directly against the merged repository state, and prepare the milestone closeout. Two pre-M6-C readiness gates remain open, required before this slice's own implementation but not authorized by this status note: (1) `scripts/connect-kubernetes-pilot.sh`'s Ctrl+C/TERM shutdown does not stop its launched API/web child processes; (2) the pilot-feedback panel discards unexported session-local judgments if closed before export, with no warning.
