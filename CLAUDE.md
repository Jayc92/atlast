...
**M0 complete (2026-07-22). M1 implementation explicitly authorized (2026-07-23) — active, slice-gated.** The documentation set is approved, the M0 tooling ADRs (0001–0013 in [docs/adr/](docs/adr/README.md)) are formally Accepted, and the full M0 foundation exists and passes CI: the monorepo workspace, the backend API shell (`apps/api`), the web application shell (`apps/web`), the shared package shells (`packages/*`), the browser acceptance suite (`tests/acceptance`), the verification tooling (`scripts/bootstrap.sh`, `scripts/verify.sh`), and the GitHub Actions workflow that runs it. The M1 architecture baseline is settled ([docs/m1-plan.md](docs/m1-plan.md) approved, ADRs 0014–0021 Accepted; ADR-0014/0015 amended by [ADR-0019](docs/adr/0019-subject-identity-and-assertion-claims.md), ADR-0017 amended by [ADR-0020](docs/adr/0020-m1-inventory-and-search-semantics.md), ADR-0016 amended by [ADR-0021](docs/adr/0021-jcs-canonicalization-clarifications.md)), and M1 implementation is authorized to execute that plan. **Slice S1 (domain schemas in `packages/shared`) is complete — merged through PR #7 on 2026-07-29. Slice S2 (repository interfaces + storage-agnostic contract-test suite skeleton) is complete — human-approved 2026-07-29 and merged through PR #10; the S2 checkpoint/HANDOFF protocol merged through PR #11 at `a7a997d` (2026-07-30). Slice S3 (fixture suite v1 in `fixtures/demo-company/`) is complete — independently reviewed and approved, merged through PR #13 at `003bccc` on 2026-07-30 with GitHub Actions `verify` passing; its closeout checkpoint merged through PR #14 at `1eca85f` (2026-07-31). Slice S4 (temporal foundations in `packages/graph-model`) was explicitly human-authorized 2026-07-31, with [ADR-0021](docs/adr/0021-jcs-canonicalization-clarifications.md) (canonicalization clarifications amending ADR-0016) accepted the same day after independent review — S4 implementation may begin only after the documentation PR recording that release merges to `main` and `main` is synchronized locally; once merged, S4 is the sole released implementation slice. No S4 implementation exists yet.** See [TASKS.md](TASKS.md) for exact per-task status and [HANDOFF.md](HANDOFF.md) for the current checkpoint.

Permitted work right now, and nothing beyond it:

- **Maintenance and corrections to the completed M0 foundation and the merged S1/S2 contract surface and S3 fixture catalog** (bug fixes, documentation fixes, dependency/security maintenance within the accepted ADRs).
- **Maintenance of the approved M1 planning documents** (corrections and review responses to [docs/m1-plan.md](docs/m1-plan.md) and ADRs 0014–0021) and of the checkpoint documentation ([HANDOFF.md](HANDOFF.md), per the checkpoint protocol below).
- **M1 implementation — Slice S4 only, effective only after its authorization PR merges to `main`.** S4 was explicitly human-authorized 2026-07-31 (ADR-0021 accepted the same day) and is limited to the scope and path boundary recorded in [TASKS.md](TASKS.md) and [HANDOFF.md § 7](HANDOFF.md): temporal-foundation primitives in `packages/graph-model/**` (Evidence total ordering by `observedAt` then `recordedSequence`, pure non-mutating ordering and horizon-selection helpers, half-open validity-interval **membership** evaluation on explicit timestamps, RFC 8785 canonical serialization per accepted ADR-0021 validated through the merged S1 `jsonValueSchema`, pure copied-array identifier-ordering helpers, SHA-256 digest primitives with lowercase hex output, and focused tests including the official RFC vectors), plus `pnpm-lock.yaml` solely for deterministic changes from the authorized package declarations (`@atlast/shared` workspace dependency; already-pinned Vitest/`@types/node` dev dependencies if required; Node.js built-ins — no new third-party runtime dependency or version upgrade), and factual TASKS.md progress notes. **S4 does NOT authorize:** changes to `packages/shared`, the S1/S2 contracts, or `fixtures/demo-company`; reconciliation or identity matching; confidence/freshness computation; derivation policy `m1-v1`; GraphAssertion derivation or payload builders; validity-interval derivation; snapshot construction, payload builders, replay, checksums-on-snapshots, or storage; repository implementations; API routes; frontend changes; connectors; or real-system access. No S4 implementation exists yet, and authorization does not imply approval of its future output — implementation, independent review, passing `scripts/verify.sh`, PR/CI, merge, and checkpoint closeout all remain required. Work is slice-driven and independently reviewed: **S5–S8 are unauthorized** until S4 is merged and the next slice is explicitly released — do not start a later slice, even preparatorily, without that release being recorded in TASKS.md.
...
- Do NOT implement M1 slices beyond the currently released one (S4, effective only on merge of its authorization PR — S5–S8 remain gated), and do NOT implement M2+ features (exploration UI, overlays, impact simulation, connectors) before their milestone is authorized.
