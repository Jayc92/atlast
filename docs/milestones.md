...
**Status:** Current — approved 2026-07-22 with the documentation set. Milestones are gated by exit criteria, not dates; a milestone is done when its criteria hold, and **each milestone requires explicit authorization**. M0 completed 2026-07-22. **M1 implementation was explicitly authorized 2026-07-23 and is active**, executing slice-gated: Slices S1–S6 are complete and merged (S6 via PR #23 at `9bf7f09`, 2026-08-10); the S6 pre-release architecture review is complete (ADR-0023 accepted 2026-08-05, the binding S6 clarification), and **Slice S6 — explicitly authorized by Joseph Carfagno on 2026-08-05 — was implemented, independently reviewed, and merged**. **Closing S6 does not authorize S7.** The S7 pre-release architecture and runtime contract is separately settled — **[ADR-0024](adr/0024-m1-query-api-runtime-contract.md) was explicitly accepted by Joseph Carfagno on 2026-08-11**, amending ADR-0017/0020 via metadata-only notices — and **Slice S7 was then explicitly authorized by Joseph Carfagno on 2026-08-11** in a separate authorization decision: S7 is the only active authorized implementation slice, effective only after the documentation PR recording the authorization merges to `main` and `main` is synchronized locally with a clean working tree; authorization does not approve future S7 implementation output. **An actual S7-B implementation attempt subsequently reproduced a genuine build-configuration contradiction, resolved by [ADR-0025](adr/0025-s7-source-alias-erasable-syntax-compatibility.md) — explicitly accepted by Joseph Carfagno on 2026-08-11, authorizing exactly one narrow refactor to `packages/graph-model/src/identity-normalization.ts` (not yet implemented).** S7 remains temporarily blocked until that acceptance record merges to `main`, the primary S7 branch is synchronized, and the refactor is implemented and verified. S8 remains gated — see the M1 section below. **M2 and later milestones remain unauthorized**, each gated on its own explicit authorization.
