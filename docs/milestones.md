...
**Status:** Current — approved 2026-07-22 with the documentation set. Milestones are gated by exit criteria, not dates; a milestone is done when its criteria hold, and **each milestone requires explicit authorization**. M0 completed 2026-07-22. **M1 is formally complete as of 2026-08-12**, having executed slice-gated: Slices S1–S8 are complete and merged (S7 via [PR #28](https://github.com/Jayc92/atlast/pull/28) at `a7624cd`, 2026-08-11; S8 via [PR #31](https://github.com/Jayc92/atlast/pull/31) at `0477cbd`, 2026-08-12); the S7 pre-release architecture and runtime contract was separately settled — **[ADR-0024](adr/0024-m1-query-api-runtime-contract.md) was explicitly accepted by Joseph Carfagno on 2026-08-11**, amending ADR-0017/0020 via metadata-only notices — and **Slice S7 was then explicitly authorized by Joseph Carfagno on 2026-08-11** in a separate authorization decision. An actual S7-B implementation attempt reproduced a genuine build-configuration contradiction, resolved by **[ADR-0025](adr/0025-s7-source-alias-erasable-syntax-compatibility.md)** — explicitly accepted by Joseph Carfagno on 2026-08-11, authorizing exactly one narrow refactor to `packages/graph-model/src/identity-normalization.ts`. **S7 was then implemented under accepted ADR-0024 as amended by ADR-0025, independently reviewed, and merged to `main` through PR #28 at `a7624cd` on 2026-08-11 with GitHub Actions `verify` passing in 2m14s.** **S1–S7 were then complete.** S7's closeout checkpoint documentation merged through PR #29 at `9acfefa` on 2026-08-11. **Closing S7 did not authorize S8.** **Joseph Carfagno then explicitly authorized M1 Slice S8 in the conductor conversation on 2026-08-11** — its authorization PR merged through PR #30 at `a4c6a5d` on 2026-08-12. **S8 was then implemented, independently reviewed, verified, merged to `main` through PR #31 at `0477cbd` on 2026-08-12 with GitHub Actions `verify` passing in 2m27s, and independently revalidated post-merge** — see the M1 section below. **S1–S8 are now complete. No implementation slice is currently active. M2 and later milestones remain unauthorized**, each gated on its own explicit authorization — **M1's completion does not imply or grant M2 authorization.**
...
**Authorization note:** M1's completion authorized M1 only — completion of M1 did not authorize M2, which requires its own separate, explicit authorization, and that authorization has not been given (see the M2 section below).
