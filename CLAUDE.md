...
- **No M1 implementation slice is currently active.** Slices S1–S3 are complete and merged; closing S3 did not authorize S4. Work is slice-driven and independently reviewed: **S4–S8 are unauthorized** until the human explicitly releases the next slice, recorded in TASKS.md — do not start a later slice, even preparatorily, without that release.
...
- Do NOT implement M1 slices without an explicit release (none is currently released — S4–S8 are gated), and do NOT implement M2+ features (exploration UI, overlays, impact simulation, connectors) before their milestone is authorized.
