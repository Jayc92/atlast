...
## M6 — Internal Pilot Validation (baseline accepted; M6-A/M6-B/readiness/packaging complete; M6-C pilot evaluated — 5/6 exit criteria PASS; M6 remains open, blocked on criterion 4)

**Status note (2026-08-24, historical):** at baseline acceptance, M6 was a candidate milestone whose **architecture baseline was Accepted but whose implementation remained unauthorized** — not yet part of the authorized-and-implemented sequence above. After M5 formally closed at checkpoint `m5-complete`, Joseph Carfagno authorized M6 planning and ADR drafting, then, after both an adversarial review and a genuinely independent final review found no substantive blocker, **explicitly accepted the complete M6 baseline: [docs/m6-plan.md](m6-plan.md) plus ADR-0039, ADR-0040, and ADR-0041** ([ADR-0042](adr/0042-m6-self-service-connector-launch-and-credential-model.md) remains Rejected, folded into [docs/m6-plan.md § 8](m6-plan.md#8-connectscan-experience) — historical record only, not part of the accepted set). At that point, this acceptance authorized no implementation, and no M6 exit criterion below was satisfied — M6-A required a separate, explicit human authorization first, after the acceptance record merged and local `main` synchronized cleanly.

**Status update (2026-08-25, historical):** **M6-A — composition-root unification and the normal-website real-Kubernetes proof — was subsequently and separately authorized by Joseph Carfagno, implemented, independently reviewed and corrected, and merged through [PR #100](https://github.com/Jayc92/atlast/pull/100); its checkpoint documentation closed through [PR #101](https://github.com/Jayc92/atlast/pull/101).** **M6-A is now complete at checkpoint `m6-a-normal-website-kubernetes-merged`** ([TASKS.md](../TASKS.md), [HANDOFF.md](../HANDOFF.md), [docs/audits/m0-synthetic-boundary-audit.md § 24](audits/m0-synthetic-boundary-audit.md)). **M6-A's completion does not complete M6 and satisfies no M6 exit criterion on its own** — M6 remains open, M6-B and M6-C remain gated and unauthorized, and no implementation slice is currently active. This section keeps this document internally consistent with [TASKS.md](../TASKS.md) and [docs/adr/README.md](adr/README.md).

**Status update (2026-08-26, historical):** **M6-B — the Kubernetes topology extension (ADR-0039: Deployments, ReplicaSets, Services, UID-based ownership matching, the full six-state Service-relationship vocabulary), the minimum visual productization, and the pilot-feedback artifact (ADR-0041) — was subsequently and separately authorized by Joseph Carfagno, implemented, independently and adversarially reviewed twice, and merged through [PR #104](https://github.com/Jayc92/atlast/pull/104); its checkpoint documentation closed through [PR #105](https://github.com/Jayc92/atlast/pull/105).** During implementation, a real Kubernetes cross-kind identity-collision defect was found and separately resolved through **Accepted [ADR-0043](adr/0043-m6-kubernetes-cross-kind-source-native-identity.md)** (merged through [PR #103](https://github.com/Jayc92/atlast/pull/103) before the corrective continuation). **M6-B is now complete at checkpoint `m6-b-kubernetes-topology-validation-merged`** ([TASKS.md](../TASKS.md), [HANDOFF.md](../HANDOFF.md), [docs/audits/m0-synthetic-boundary-audit.md § 25](audits/m0-synthetic-boundary-audit.md)). **M6-B's completion does not complete M6 and satisfies no M6 exit criterion on its own** — every one of the six ([docs/m6-plan.md § 15](m6-plan.md#15-m6-exit-criteria)) requires an actual unaided-tester run, which only M6-C can provide. M6 remains open, M6-C remains gated and unauthorized, and no implementation slice is currently active. Two pre-M6-C readiness gates remain open, required before the employee pilot but not before this closure: pilot-script process-tree shutdown, and pilot-feedback session-state preservation/discard protection on panel close.

**Status update (2026-08-27, historical):** **Pre-M6-C readiness hardening — closing both gates named immediately above — was subsequently and separately authorized by Joseph Carfagno, implemented, independently and adversarially reviewed, corrected, and merged through [PR #107](https://github.com/Jayc92/atlast/pull/107) at `2dd7b80e2ca911f66eebee65b696ef8639294531`.** Both gates now evaluate **PASS**: the pilot script's Ctrl+C/TERM shutdown now reaches every launched API/web child process (proven live with a genuine terminal Ctrl+C and a separate SIGTERM run), and the pilot-feedback panel now preserves unexported judgments, tester role, and session notes across a close/reopen cycle. **Checkpoint `m6-c-readiness-complete`** ([TASKS.md](../TASKS.md), [HANDOFF.md](../HANDOFF.md), [docs/audits/m0-synthetic-boundary-audit.md § 26](audits/m0-synthetic-boundary-audit.md)). **This readiness closure does not complete M6 and satisfies no M6 exit criterion on its own** — M6 remains open, M6-C remains gated and unauthorized, and no implementation slice is currently active.

**Status update (2026-08-28–2026-08-31, current):** Two pilot-packaging correction slices closed fresh-clone onboarding and build-workspace gaps, merged through [PR #109](https://github.com/Jayc92/atlast/pull/109) and [PR #110](https://github.com/Jayc92/atlast/pull/110) (`docs/audits/m0-synthetic-boundary-audit.md § 27`). **M6-C — the deterministic pilot sandbox and real-tester acceptance run — was executed: a real, unaided internal employee ran the pilot on 2026-08-28** (session `608f7d89-4b68-414f-892b-0b36cd64ddd7`, `developerIntervention.occurred: false`), producing a versioned pilot-feedback artifact per Accepted [ADR-0041](adr/0041-m6-pilot-feedback-storage-boundary.md). **No contemporaneously durable M6-C slice-authorization record existed anywhere in the repository at the time of that session.** This sequencing irregularity was discovered during formal evaluation of the pilot against the six exit criteria; **on 2026-08-31 the human maintainer (Joseph Carfagno) explicitly acknowledged and authorized M6-C as a slice, for governance purposes — this authorization is not backdated to 2026-08-28**. The pilot's evidentiary content is evaluated strictly on its actual merits, independent of that sequencing irregularity, which is disclosed rather than concealed. **Evaluated directly against the exact six Accepted exit criteria ([§ 15](#15-m6-exit-criteria)) and merged to `main` through [PR #111](https://github.com/Jayc92/atlast/pull/111) at checkpoint `m6-c-pilot-evaluated`** ([TASKS.md](../TASKS.md); [docs/audits/m0-synthetic-boundary-audit.md § 28](audits/m0-synthetic-boundary-audit.md)): **criteria 1, 2, 3, 5, and 6 evaluate PASS; criterion 4 evaluates FAIL** — the tester observed and correctly judged a deliberately-built known-zero-match Service, but the accepted verdict vocabulary provides no relationship record to attach a `known-zero` verdict to, so that applicable, observed verdict could not actually be recorded. **M6-C is complete as an executed, evaluated pilot slice. This does not complete M6**: no milestone in this project's history has closed with a failing exit criterion, and the Accepted baseline contains no provision for deferring one. **M6 remains open, blocked specifically on criterion 4.** No pilot finding was fixed by this evaluation; no M7 work is authorized or implied.

**Goal (accepted baseline; implemented and executed — see the 2026-08-28–2026-08-31 status update above):** Prove that a technically competent internal employee who did not build Atlast can independently connect an approved disposable local Kubernetes sandbox, see the real discovered topology in the browser, judge its accuracy, run a hypothetical change through the existing impact workflow, and judge the resulting analysis — end to end, without a developer driving it for them. This is a product-testability proof, not a broad accuracy or scale claim ([docs/m6-plan.md § 3](m6-plan.md#3-target-tester-and-milestone-purpose)). The baseline architecture was implemented across M6-A/M6-B/readiness/packaging, and a real employee pilot was executed and evaluated (checkpoint `m6-c-pilot-evaluated`, [PR #111](https://github.com/Jayc92/atlast/pull/111)) — **M6 itself remains open**, blocked on criterion 4 pending a targeted independent employee validation of the [PR #112](https://github.com/Jayc92/atlast/pull/112) remediation.

**Scope (accepted baseline; implemented — M6-A/M6-B/readiness/packaging complete):** the minimum truthful Kubernetes topology beyond M5's Pods-only coverage (Deployment→ReplicaSet→Pod, Service→Pod, both via Kubernetes-native facts only); unifying connector-derived Evidence with the normal browser-facing application, since it was previously invisible to it; a minimum visual source/freshness distinction; a self-service CLI connect/scan flow; and a pilot-evaluation artifact kept structurally separate from the Evidence/domain model.
...
**Exit criteria:** see [docs/m6-plan.md § 15](m6-plan.md#15-m6-exit-criteria) for the exact, Accepted six-criterion text, left unedited there as the frozen baseline contract. Evaluated directly against the real M6-C pilot session above: **criteria 1, 2, 3, 5, and 6 evaluate PASS; criterion 4 evaluates FAIL** (see the 2026-08-28–2026-08-31 status update above and [audit § 28](audits/m0-synthetic-boundary-audit.md) for the full evaluation). M6 does not yet meet all six exit criteria and therefore **remains open**.
