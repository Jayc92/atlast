...
**Status:** Accepted — amends ADR-0017 and ADR-0020 (metadata-only notices; their accepted decision text is preserved); amended by ADR-0025
...
> **Amendment notice (2026-08-11):** [ADR-0025](0025-s7-source-alias-erasable-syntax-compatibility.md) (**Accepted** 2026-08-11) amends this ADR via a metadata-only notice, extending the § 14/§ 15 build strategy and the "Exact S7 Boundary" section's authorized-file list — **only** to add `packages/graph-model/src/identity-normalization.ts`, scoped **only** to the exact class-declaration-syntax refactor ADR-0025 describes (`IdentityNormalizationError`'s two constructor parameter properties replaced with `declare readonly` fields plus explicit constructor assignment). This closes a build-configuration gap this ADR's own three correction passes did not anticipate: § 14 step 5's `@atlast/graph-model` source alias, once actually exercised by an import, applies `apps/api`'s `erasableSyntaxOnly` compiler option (ADR-0011) to that package's entire source, and `identity-normalization.ts`'s parameter properties fail that check (`TS1294`). This ADR's decision text below — including its own § 14/§ 15 and Exact S7 Boundary content — is **preserved verbatim**; ADR-0025 is the sole normative source for the one-file extension it adds.
