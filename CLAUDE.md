...
- **M1 implementation — Slice S5 only, effective only after its acceptance record merges to `main`.** S5 was human-authorized 2026-07-31 with ADR-0022 explicitly accepted 2026-08-05, and is limited to the exact accepted [ADR-0022](docs/adr/0022-m1-reconciliation-policy-and-assertion-derivation.md) boundary recorded in [TASKS.md](TASKS.md) and [HANDOFF.md § 7](HANDOFF.md): the `m1-v1` derivation policy; normalization and type-free stable identifier construction; aliases and ambiguity; event-time standing-claim reconciliation; standing-source-filtered provenance; confidence and conflict derivation; assertion validity and complete revision history; schema-valid rule traces; content-addressed GraphAssertion construction through the S4 primitives; the pure reconciliation function; freshness classification; and focused tests — **only within the ADR-0022 § 14 paths** (`packages/graph-model/src/**` plus factual TASKS.md notes; no manifest, lockfile, or dependency change). **S5 does NOT authorize:** changes to `packages/shared` or S1/S2 contracts, fixture JSON, storage or repository implementations, snapshots or replay, query/API routes, frontend, connectors, or infrastructure. No S5 implementation exists yet, and authorization does not imply approval of its future output — implementation, independent review, passing `scripts/verify.sh`, PR/CI, merge, and checkpoint closeout all remain required. Work is slice-driven and independently reviewed: **S6–S8 are unauthorized** until S5 is merged and the next slice is explicitly released — do not start a later slice, even preparatorily, without that release being recorded in TASKS.md.
...
- Do NOT implement M1 slices beyond the currently released one (S5, effective only on merge of its acceptance record — S6–S8 remain gated), and do NOT implement M2+ features (exploration UI, overlays, impact simulation, connectors) before their milestone is authorized.
