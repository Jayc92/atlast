...
- [~] **S2 — Repository interfaces + contract-test suite skeleton** — **authorized 2026-07-29 (released after S1 merged via PR #7) — the only active authorized slice**; **implemented and awaiting human review**. Scope: **async repository interfaces in `packages/shared`** carrying the **full M1 repository contract** — reads pinned by (asOf, horizon, derivationVersion); bounded collection reads and bounded traversal; Evidence lookup and evidence-chain contracts; subject and GraphAssertion revision reads; conflict and ambiguity retrieval; the relationship endpoint referential-integrity obligations (per ADR-0019 § 4, defined here and proven by the implementing slice) — plus a **reusable, storage-agnostic contract-test suite skeleton**. S2 does **not** authorize: an in-memory or database repository implementation (S6); fixture creation (S3); canonical serialization or hashing (S4/S6); temporal algorithms (S4); reconciliation (S5); API routes (S7); frontend changes; or any S3–S8 or M2+ work
  - [~] **S2 contract remediation per [ADR-0020](docs/adr/0020-m1-inventory-and-search-semantics.md) — accepted by human review 2026-07-29**: S2 human review had found ADR-0017's inventory wording ("filter by type and status") and search wording ("identifiers and names") contradicting the accepted domain model (ADR-0014 as amended by ADR-0019); **that architecture ambiguity is resolved** — ADR-0020 (Accepted, amending only that ADR-0017 wording) fixes entity-only inventory with an optional claim-level `entityType` filter (match-by-any-claim under conflict, no winner implied), no generic "status" filter, and identifier-only search over complete canonical identifiers with locale-independent normalization. **S2 remains implemented and awaiting its final remediation and review: the ADR-0020 contract corrections must be applied before S2 can be approved** (schema-validated `entityType` filter, bounded filtered inventory read, conflicting-type-claim visibility and match-by-any-claim contract cases, identifier-only search wording and tests). ADR-0020's acceptance released only that remediation — it did not approve S2 itself; **S3–S8 remain gated** on their own explicit per-slice releases
